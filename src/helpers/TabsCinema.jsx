...
const TabsCinema = (props) => {
  ...
  const layGioChieu = (lstLichChieuTheoPhimarr) => {
    return lstLichChieuTheoPhimarr.map((item, index) => {
      ...
      return (
        <NavLink
          to={isLogin ? `/booking-tickets/${item.maLichChieu}` : `/login`}
          key={index}
        >
          <Tag style={{ fontSize: "1.3rem", padding: "8px" }} color="green">
            {gioChieu}
          </Tag>
        </NavLink>
      );
  ...
  const renderCumRap = (arrCumRap) => {
    ...
    const currentCumRap =
      arrCumRap?.slice(indexOfFirstCumRap, indexOfLastCumRap) || [];
    const totalCumRapPages = Math.ceil(
      (arrCumRap?.length || 0) / cumRapPerPage
    );
    ...
    return (
      <div>
              ...
              label: (
                <div className="flex relative gap-2">
                  <img
                    ...
                    src={
                      "https://thumbs.dreamstime.com/b/default-image-icon-vector-missing-picture-page-website-design-mobile-app-no-photo-available-235041328.jpg"
                    }
                  />
                  <div className="flex flex-col items-start">
                    <p className="text-xl font-bold">{item.tenCumRap}</p>
                    <p className="text-lg text-start text-wrap w-96">
                      {item.diaChi}
                    </p>
                  </div>
                </div>
        ...
        {totalCumRapPages > 1 && (
          <div
            className="flex justify-start mt-4 absolute"
            style={{ top: "50%" }}
          >
            <Space>
              {Array.from({ length: totalCumRapPages }, (_, i) => i + 1).map(
                (page) => (
                  <button
                    key={page}
                    onClick={() => handleCumRapPageChange(page)}
                    className={`px-3 py-1 rounded ${
                      cumRapCurrentPage === page
  ...
  const renderListPhim = (arrPhim) => {
    ...
    const filteredMovies = arrPhim.filter(
      (item) => item.dangChieu && item.sapChieu
    );
