...
const DetailMoviePage = () => {
  ...
  const openNotification = () => {
    api.info({
      message: `Thông báo`,
      description: "Video trailer đang được cập nhập. Vui lòng quay lại sau!",
      placement: "top",
    });
  };
  ...
  useEffect(() => {
    apiMovies.getMovieDetail(id).then((res) => {
      setMovieDetail(res.data.content);
    });
    apiMovies.getShowTimeMovie(id).then((res) => {
  ...
  const layGioChieu = (gioChieuPhim) => {
    ...
    return (
      <div>
        <NavLink
          to={`${
            isLogin ? `/booking-tickets/${gioChieuPhim.maLichChieu}` : "/login"
          }`}
          className="border-2 p-1 text-lg rounded-xl"
        >
          {gioChieu}
        </NavLink>
      </div>
    ...
    <>
      ...
      <div className="container mx-auto mt-40 detail-movie-page">
        <div className="flex justify-around">
          ...
          />
          <div className="w-2/3">
            <Divider style={{ border: "2px" }}>
              <Title level={1}>{movieDetail?.tenPhim}</Title>
            </Divider>
            <div className="flex flex-col justify-between h-2/3 gap-2">
              <div className="flex items-center">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <p className="text-gray-500 text-wrap w-3/4">
                  ...
                  format={(percent) => {
                    return (
                      <div className="">
                        ...
                        />
                        <div className="text-black">{percent / 10 + "/10"}</div>
                      </div>
                    );
                  }}
                />
